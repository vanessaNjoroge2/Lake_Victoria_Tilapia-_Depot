...
class User
{
    ...
    // UPDATE - Store a password reset token
    public function setResetToken($user_id, $token, $expires)
    {
        // Ensure reset_token columns exist (auto-migration)
        try {
            $check = $this->conn->query("SHOW COLUMNS FROM " . $this->table . " LIKE 'reset_token'");
            if ($check->rowCount() === 0) {
                $this->conn->exec("ALTER TABLE " . $this->table . "
    ...
    // READ - Find a user by a valid (unexpired) reset token
    ...
    // UPDATE - Clear reset token after it has been used
    ...
    // UPDATE - Set a new password by user ID (used after token validation)
